...
fhandler = logging.FileHandler(filename='experiments/23JAN/23janciteseeroneshot/CiteseerALLoneshot%.log', mode='a')
...
oldadj, features, labels, idx_train, idx_val, idx_test = load_data('citeseer')
...
for i in range(100):
        print(f"Starting pruning iteration = {i}")
        ...
        Gnew = nx.from_scipy_sparse_matrix(adj)
        print(Gnew)
        print("")
        print("===========================================================")
        with open("experiments/23JAN/23janciteseeroneshot/InfoCiteseerALLoneshot%.txt", "a") as f:
                    print(f"Starting iteration - {i}",file=f)  
        ...
        # if(sparsity >= 85):
        #     print("Achieved required sparsity. Stopping Pruning...")
        #     break
...
if __name__ == "__main__":
    ...
    for p in range(20):
        ...
        print("syd : Sparsity:[{}], Best Val:[{:.2f}] at epoch:[{}] | Final Test Acc:[{:.2f}] Adj:[{:.2f}%] Wei:[{:.2f}%]"
            .format(p + 1, best_acc_val * 100, final_epoch_list, final_acc_test * 100, sparsity, 100-wei_spar))
        print("=" * 120)
