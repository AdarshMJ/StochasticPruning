...
fhandler = logging.FileHandler(filename='experiments/Coraonetimerecursive0.9.log', mode='a')
...
for i in range(20):
      logging.info(f'{i} - Iterative pruning with Pdel = {pdel}')
      adj = stochastic_pruning(oldadj, pdel)
      oldadj = adj
    ...
    def parser_loader():
     ...
     if __name__ == "__main__":
          ...
          for p in range(20):
              ...
              logging.info(("syd : Sparsity:[{}], Best Val:[{:.2f}] at epoch:[{}] | Final Test Acc:[{:.2f}] Adj:[{:.2f}%] Wei:[{:.2f}%]"
                  .format(p + 1, best_acc_val * 100, final_epoch_list, final_acc_test * 100, adj_spar, wei_spar)))
              print("=" * 120)
